...
initial_guess_real = [0, 0, 0]  
bounds_real = [(25, 35), (0, 12), (80, 120)]  


initial_guess_bounded = [(2 * (initial_guess_real[i] - bounds_real[i][0]) / (bounds_real[i][1] - bounds_real[i][0])) - 1 
                         for i in range(len(initial_guess_real))]
bounds_bounded = [(-1, 1) for _ in range(len(bounds_real))]

result = minimize(lambda x: -Ti(x , coefficients_TI), initial_guess_bounded, bounds=bounds_bounded)
...
result = minimize(lambda x: -F(x , coefficients_F), initial_guess_real, bounds=bounds)
...
result = minimize(lambda x: g(x , coefficients_G), initial_guess_bounded, bounds=bounds_bounded)
